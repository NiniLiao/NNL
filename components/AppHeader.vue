...
const isSolidHeader = computed(() => route?.path === '/wishlist' || (route.path && route.path.startsWith('/account')))
...
const handleMobileClick = (item) => {
    if (menuData[item.key]) {
        mobileSubMenu.value = item.key
        accordions.col1 = true
        accordions.col2 = false
    } else {
...
const toggleAccordion = (col) => {
    accordions[col] = !accordions[col]
}

const menuItems = [
  { name: 'Black Friday Sale', key: 'sale', link: '/' },
  { name: 'New', key: 'new', link: '/' },
  { name: 'Best Sellers', key: 'best-sellers', link: '/' },
  { name: 'Designers', key: 'designers', link: '/' },
  { name: 'Bags', key: 'bags', link: '/' },
  { name: 'Clothing', key: 'clothing', link: '/' },
  { name: 'Denim', key: 'denim', link: '/' },
  { name: 'Editorial', key: 'editorial', link: '/' },
  { name: 'About', key: 'about', link: '/' },
]

const menuData = {
    bags: {
        col1: { title: 'Size', links: ['Small Bags', 'Medium Bags', 'Large Bags', 'View All'] },
        col2: { title: 'Category', links: ['Bucket Bags', 'Shoulder Bags', 'Tote Bags', 'Accessories', 'View All'] },
        image1: { src: 'https://images.unsplash.com/photo-1590874103328-eac38a683ce7?w=500&q=80', text: 'Suede Edit' },
        image2: null,
        promo: { discount: '15', desc: 'Only bag discount of the season.', footer: 'Shop Black Friday' }
    },
    denim: {
        col1: { title: 'All Jeans', links: ['Wide-Leg Jeans', 'Straight Jeans', 'Barrel Jeans', 'Relaxed Jeans', 'Flare & Bootcut & Skinny'] },
        col2: { title: 'All Denim', links: ['Denim Dresses & Skirts', 'Denim Jackets', 'Denim Shirts', 'Denim Shorts'] },
        image1: { src: 'https://images.unsplash.com/photo-1541099649105-f69ad21f3246?w=500&q=80', text: 'All Denim' },
        image2: { src: 'https://images.unsplash.com/photo-1584370848010-d7cc637703ef?w=500&q=80', text: 'New Denim' },
        promo: { discount: '60', desc: 'Designer denim at our best prices this year.', footer: 'Shop Black Friday' }
    },
    clothing: {
        col1: { title: 'All Clothing', links: ['Jackets & Blazers', 'Denim', 'Dresses', 'Coats & Puffer', 'Pants', 'Sweaters'] },
        col2: { title: 'Featured', links: ['Short Coats', 'Burgundy & Oxblood', 'Suede & Corduroys', 'New Looks', 'Fall Essentials'] },
        image1: { src: 'https://images.unsplash.com/photo-1542272201-b1ca555f8505?w=500&q=80', text: 'Coats' },
        image2: { src: 'https://images.unsplash.com/photo-1551488852-7a304fa73846?w=500&q=80', text: 'Jackets' },
        promo: { discount: '60', desc: "The event you don't want to miss.", footer: 'Shop Black Friday' }
    }
}

const activeContent = computed(() => {
    const key = mobileSubMenu.value || activeMenu.value
  ...
  <div>
    <header class="absolute top-0 left-0 w-full z-40 transition-colors duration-200 border-b border-transparent" :class="[(isHovered || activeMenu || isMobileMenuOpen || isSearchOpen || isSolidHeader) ? 'bg-white text-black border-gray-100 shadow-sm' : 'bg-transparent text-white hover:bg-white hover:text-black']" @mouseenter="isHovered = true" @mouseleave="handleMouseLeave">
      ...
      <div class="flex items-center justify-between px-4 md:px-8 h-14 md:h-16 w-full max-w-[1920px] mx-auto relative">
        <div v-if="isSearchOpen" class="w-full h-full flex items-center justify-center absolute inset-0 bg-white z-50 px-4">
        ...
        <div v-else class="w-full flex items-center justify-between h-full">
            <button class="xl:hidden p-2 -ml-2 hover:opacity-70" @click="isMobileMenuOpen = true">
    ...
    <div v-if="isMobileMenuOpen" class="fixed inset-y-0 left-0 w-full max-w-md bg-white z-[100] flex flex-col xl:hidden overflow-hidden text-primary">
        <div class="w-full text-center py-2 px-2 bg-[#0B1215] text-white flex-shrink-0"><p class="text-[9px] uppercase tracking-[0.15em] font-medium">BLACK FRIDAY UP TO 60% OFF | BIGGEST EVENT...</p></div>
        <div v-if="!mobileSubMenu" class="flex flex-col h-full overflow-hidden">
            <div class="flex items-center justify-between px-4 h-16 border-b border-gray-100 flex-shrink-0">
                 ...
                 <div class="flex items-center gap-2">
                    ...
                    <span class="absolute -top-1 -right-1 w-2 h-2 bg-accent rounded-full border border-white text-[8px] flex items-center justify-center text-white font-bold">1</span>
                 </div>
            </div>
        ...
        <div v-else class="flex flex-col h-full bg-white text-primary overflow-hidden">
            <div class="flex items-center justify-between px-4 h-16 border-b border-gray-100 flex-shrink-0">
                 ...
                 <div class="flex items-center gap-2"><button @click="isMobileMenuOpen = false; toggleSearch()"><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path></svg></button><NuxtLink to="/account/login" @click="isMobileMenuOpen = false"><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"></path></svg></NuxtLink><NuxtLink to="/wishlist" @click="isMobileMenuOpen = false"><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z"></path></svg></NuxtLink><span class="absolute -top-1 -right-1 w-2 h-2 bg-accent rounded-full border border-white text-[8px] flex items-center justify-center text-white font-bold">1</span></div>
            </div>
